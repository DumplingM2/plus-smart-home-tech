...
public class ShoppingCartController implements CartClient {
    ...
    @PutMapping
    public ShoppingCartDto addProductsInCart(@RequestParam String username, @RequestBody Map<UUID, Long> newProducts) throws FeignException {
    ...
    @GetMapping
    public ShoppingCartDto getActiveShoppingCartOfUser(@RequestParam String username) throws FeignException {
    ...
    @DeleteMapping
    public void deactivateCart(@RequestParam String username) throws FeignException {
        log.info("Запрос на деактивацию корзины товаров");
        shoppingCartService.deactivateCart(username);
    }

    @PostMapping("/remove")
    public ShoppingCartDto removeOtherProductsFromCart(@RequestParam String username, @RequestBody List<UUID> productIds) throws FeignException {
